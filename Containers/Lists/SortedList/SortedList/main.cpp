...
int main(void){
    ...
    if(passtest){
        ...
        for(int i=0;i<20 && passtest;i++){
            if(mirror[i] != *(it++)){
                passtest=false;
                cout << "Error 4a: postfix ++ (it++) operator is not returning iterator to correct node at: " << i << ". Where mirror[" << i << "] = " << mirror[i] << ", while yours returns: " << *(--it) << endl;
            }
        }
    ...
    if(passtest){
        ...
        for(int i=0;i<20;i++){
            ...
            auto it = recListRef.search(mirror[i]);
